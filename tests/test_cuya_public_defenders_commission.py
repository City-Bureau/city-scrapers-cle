...
def test_id():
    assert (
        parsed_item["id"]
        == "cuya_public_defenders_commission/201909041630/x/public_defenders_commission"
    )
...
def test_source():
    assert (
        parsed_item["source"]
        == "http://publicdefender.cuyahogacounty.us/en-US/09042019CommissionMeeting.aspx"  # noqa
    )
